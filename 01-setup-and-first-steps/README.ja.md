...
id: CopilotCLI-01
title: はじめてのステップ
description: ハンズオンデモを通じて GitHub Copilot CLI を体験し、インタラクティブモード、プランモード、プログラムモードの使い分けを学びます。
...
<img src="assets/first-copilot-experience.png" alt="デスクに座る開発者。モニターにはコードが表示され、AIの支援を表す光の粒子が舞っている" width="800"/>
...
<img src="assets/modes-and-commands.png" alt="Copilot CLI のモードとコマンドを表す、光る画面やダイヤル、イコライザーを備えた未来的なコントロールパネル" width="800"/>
...
<img src="assets/ordering-food-analogy.png" alt="GitHub Copilot CLI の3つの使い方 - Plan モード（GPSルート）、Interactive モード（店員との会話）、Programmatic モード（ドライブスルー）" width="800"/>
...
<img src="assets/interactive-mode.png" alt="Interactive モード - 質問に答えたり注文を調整したりしてくれる店員との会話のようなもの" width="250"/>
...
<img src="assets/plan-mode.png" alt="Plan モード - GPSを使って旅行前にルートを計画するようなもの" width="250"/>
...
<img src="assets/programmatic-mode.png" alt="Programmatic モード - 素早い注文のためにドライブスルーを使うようなもの" width="250"/>
...
| `/compact` | 会話を要約してコンテキスト使用量を削減（オプションでフォーカス指示を追加可能。例: `/compact focus on the bug list`） |
| `/context` | コンテキストウィンドウのトークン使用量と可視化を表示 |
| `/keep-alive` | Copilot CLI がアクティブな間、システムがスリープするのを防止 — ノート PC などで長時間かかるタスクを実行する際に便利 |
| `/memory [on\|off\|show]` | 永続メモリの有効化、無効化、または表示 — すべてのセッション間で記憶される事実や設定 |
...
| `/usage` | セッションの使用状況メトリクスと統計（クォータの進捗バーを含む）を表示 |
...
| `/voice` | ローカルの音声認識（Speech-to-Text）を使用してプロンプトをディクテーションする — 入力する代わりに自然に話しかける |
